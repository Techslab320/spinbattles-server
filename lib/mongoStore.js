...
export async function getCandidateByEmail(email) {
  return Candidate.findOne({ email: String(email).toLowerCase().trim() })
}

export async function createCandidate({ email, password, firstName, lastName }) {
...
export async function setCandidateOtp(email, otp) {
  const normalizedEmail = String(email).toLowerCase().trim()
  const expiresAt = new Date(Date.now() + 10 * 60 * 1000)
  const updated = await Candidate.findOneAndUpdate(
    { email: normalizedEmail },
    { otpHash: hashPassword(String(otp)), otpExpiresAt: expiresAt },
    { new: true }
  )
  if (!updated) {
...
export async function verifyCandidateOtp(email, otp) {
  const candidate = await getCandidateByEmail(email)
  if (!candidate) return { ok: false, reason: 'not_found' }
  if (candidate.emailVerified) return { ok: true, alreadyVerified: true }
  if (!candidate.otpHash || !candidate.otpExpiresAt) {
    return { ok: false, reason: 'no_otp' }
  }
  if (new Date() > candidate.otpExpiresAt) {
    return { ok: false, reason: 'expired' }
  }
  if (!verifyPassword(String(otp), candidate.otpHash)) {
    return { ok: false, reason: 'invalid' }
  }
  candidate.emailVerified = true
  candidate.otpHash = undefined
  candidate.otpExpiresAt = undefined
